...
import { MultipleChoiceQuizCard } from "@/components/agents/multiple-choice-quiz";
import {
  CodeBlock,
  CodeBlockHeader,
  CodeBlockIcon,
...
export type MarkdownProps = {
  ...
  onQuizSubmit?: (message: string) => Promise<boolean>;
  streaming?: boolean;
};
...
const MarkdownQuizSubmitContext = React.createContext<MarkdownProps["onQuizSubmit"] | undefined>(undefined);
...
type MarkdownCodeRendererProps = {
  ...
  onQuizSubmit?: MarkdownProps["onQuizSubmit"];
  streaming: boolean;
};
...
function renderMarkdownCode({
      ...
      return (
        <CodeBlock className="border-border/70 my-1 shadow-none dark:border-white/15">
          <CodeBlockHeader className="border-b border-border/60 dark:border-white/10">
            ...
            <CodeBlockContent className="whitespace-normal bg-muted/50 dark:bg-black/25">
  ...
  if (onQuizSubmit && isMultipleChoiceQuizFence(rawLanguage)) {
    const quiz = parseMultipleChoiceQuiz(code);
    if (
      streaming &&
      (quiz || isStreamingMultipleChoiceQuiz(code, rawLanguage))
    ) {
      return (
        <div className="not-prose my-4">
          <ReasoningText phrases={["Generating quiz"]} variant="swap" />
        </div>
      );
    }
    if (quiz) {
      return <MultipleChoiceQuizCard quiz={quiz} onSubmit={onQuizSubmit} />;
    }
  }
  ...
  img: ({ src, alt }) => {
    ...
    return (
      <div className="group/img relative w-full max-w-lg overflow-hidden rounded-[20px] border border-[#E5E5E5] bg-gray-50 shadow-sm dark:border-gray-700 dark:bg-[#2f2f2f]">
        {/* Markdown images may use remote URLs that are not configured for next/image. */}
    ...
    <MarkdownRenderContext.Consumer>
      {({ inTableCell }) => (
        <MarkdownCanvasActionContext.Consumer>
          {(onConvertToCanvas) => (
            <MarkdownQuizSubmitContext.Consumer>
              {(onQuizSubmit) => (
                <MarkdownStreamingContext.Consumer>
                  {(streaming) =>
                    renderMarkdownCode({
                      className,
                      children,
...
function MarkdownComponent({
  ...
  const supportsInteractiveQuizzes = Boolean(onQuizSubmit);
  ...
  const processedContent = useMemo(() => {
    const sourceText = supportsInteractiveQuizzes
      ? stripManualQuizSubmissionInstructions(children)
      : children;
  ...
  }, [children, supportsInteractiveQuizzes]);
  ...
  return (
    <div className={cn("prose prose-neutral dark:prose-invert max-w-none break-words prose-p:leading-relaxed prose-pre:bg-muted prose-pre:border prose-pre:overflow-x-auto prose-code:before:content-none prose-code:after:content-none prose-headings:text-foreground prose-strong:text-foreground", className)}>
      <MarkdownCanvasActionContext.Provider value={onConvertToCanvas}>
        <MarkdownQuizSubmitContext.Provider value={onQuizSubmit}>
        ...
        </MarkdownQuizSubmitContext.Provider>
